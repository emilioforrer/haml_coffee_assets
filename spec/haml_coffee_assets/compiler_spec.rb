...
(function(context) {
  ...
  context 'extend the scope' do
    it 'uses the context for the scope for a haml coffee template' do
      HamlCoffeeAssets.config.extendScope = true
      HamlCoffeeAssets::Compiler.compile('test', '%h2= name').should eql <<-TEMPLATE
(function() {
  var _ref;
